...
def register(user, session: Session = Depends()):
    ...
    try:
        ...
        # Validate the request body using the Pydantic model
...
@router.post("/login")
def login(user_credentials, session: Session = Depends()):
    # Import everything inside the function to avoid early initialization
    try:
        from ..dependencies.database import get_db_session
        ...
        # Validate the request body using the Pydantic model
        validated_credentials = UserLogin.model_validate(user_credentials)
    ...
    user = authenticate_user(session, validated_credentials.email, validated_credentials.password)
...
try:
    ...
    for route in router.routes:
        if route.path == "/register" and hasattr(route, 'methods') and "POST" in route.methods:
            route.response_model = Token
        elif route.path == "/login" and hasattr(route, 'methods') and "POST" in route.methods:
            route.response_model = Token
except:
