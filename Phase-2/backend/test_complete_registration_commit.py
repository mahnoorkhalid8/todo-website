#!/usr/bin/env python3
"""
Test script to simulate the complete registration process step by step with commit
"""
...
def test_complete_registration():
    """Test the complete registration process step by step with commit"""
    print("Testing complete registration process with commit...")
    ...
    print("\nStep 5: Creating user via service with commit...")
    try:
        with Session(engine) as session:
            db_user = create_user(session, email, password, name)
            session.commit()  # This was missing in the previous test
